...
/* ─────────────────────────────────────────────────────────
 * LIVELINE (inlined) — Grove does not carry the `liveline`
 * package, so the static snapshot rendering it was used for
 * here is a small SVG line chart with the same prop surface.
 * Live streaming, momentum and pulse are accepted but inert:
 * every usage in this file renders a paused snapshot.
 * ───────────────────────────────────────────────────────── */
...
function Liveline({
  ...
  const lines = useMemo(() => {
    if (series && series.length > 0) {
      return series.map((s) => ({ points: s.data, color: s.color }));
    }
    if (data && data.length > 0) {
      return [{ points: data, color: color ?? "var(--red)" }];
    }
    return [];
  }, [series, data, color]);
  ...
  return (
    ...
    >
      ...
      {lines.map((line, li) => {
        ...
        if (n < 2) return null;
        const d = line.points
          .map((p, i) => `${i === 0 ? "M" : "L"}${x(i, n).toFixed(2)},${y(p.value).toFixed(2)}`)
          .join(" ");
        return (
          <g key={li}>
            {fill && <path d={`${d} L${W},${H} L0,${H} Z`} fill={line.color} opacity={0.08} />}
            <path
              d={d}
              fill="none"
              stroke={line.color}
    ...
    <div className="min-h-[278px] rounded-card bg-surface p-3 shadow-hairline">
      ...
      <div className="mt-3 min-h-16 rounded-control bg-inset px-2.5 py-2 shadow-hairline">
        ...
        <span className="mt-1 block text-[11px] leading-relaxed text-ink-3">
          Contribution snapshot across current inventory value. Segment selection changes the
          inspected group without moving the card.
        </span>
      </div>
    </div>
  );
