...
const COLUMN_META: Record<string, ColumnMeta> = {
  ...
  Categories: {
    ...
    toolKind: "model",
    inputs: "Company",
    prompt: { before: "Tag each ", chip: "Company", after: " with its market categories." },
  },
  ...
  "Connection strength": {
    ...
    prompt: { before: "Score the relationship from ", chip: "Last interaction", after: "." },
  },
  Links: {
    type: "URL",
    tool: "Web lookup",
...
const MODEL_OPTIONS = ["Local model"];
...
function Tag({ name }: { name: string }) {
  ...
  return (
    <span className="records-tag" style={{ "--tag-base": color.base } as React.CSSProperties}>
      {name}
    </span>
  );
}
      ...
      <button type="button" className="records-header-button" onClick={onPick}>
        ...
        {sortKey && (
          <span
            ...
            style={{
              transform: sort.key === sortKey && sort.dir === -1 ? "rotate(180deg)" : undefined,
            }}
          >
            <Icon size={12}>
...
export default function RecordsTable({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () =>
        setCalc((current) => (current ? { ...current, resolved: current.resolved + 1 } : current)),
      110,
    );
  ...
  const selectedInputs =
    prop && meta ? (inputSelections[prop.col] ?? (meta.inputs ? [meta.inputs] : [])) : [];
  const tableWidth =
    ...
    <div className={`records-shell${fill ? " is-fill" : ""}`}>
      ...
      >
        <table
          ...
          style={{ width: columnWidthsLocked ? tableWidth : "100%", minWidth: tableWidth }}
        >
          <colgroup>
          ...
          <thead>
            <tr>
              <th
                className={`records-header-cell records-sticky-cell ${prop?.col === "Company" ? "is-colsel" : ""}`}
              >
                <div
                  ...
                  <button
                    ...
                    onClick={(event) => {
                      ...
                      setAddOpen((current) =>
                        current
                          ? null
                          : { x: Math.min(rect.left, window.innerWidth - 276), y: rect.bottom + 6 },
                      );
                    }}
          ...
          <tbody data-sound-silent>
            {visibleRows.map((row, index) => {
              ...
              return (
                <tr key={row.id} className={`records-row ${selectedRow ? "is-selected" : ""}`}>
                  <td
                    className={`records-cell records-sticky-cell records-company-cell ${prop?.col === "Company" ? "is-colsel" : ""}`}
                  >
                  ...
                  <td className={`records-cell ${prop?.col === "Categories" ? "is-colsel" : ""}`}>
                    {isCalc("Categories", index) ? <CalcCell /> : <TagList tags={row.tags} />}
                  </td>
                  <td
                    className={`records-cell ${row.last === "No contact" ? "records-muted" : ""} ${prop?.col === "Last interaction" ? "is-colsel" : ""}`}
                  >
                    {isCalc("Last interaction", index) ? <CalcCell /> : row.last}
                  </td>
                  <td
                  ...
                  <td className={`records-cell ${prop?.col === "Links" ? "is-colsel" : ""}`}>
                    {isCalc("Links", index) ? (
                      <CalcCell />
                    ) : row.website ? (
                      <a
                        ...
                        rel="noreferrer"
                      >
                        <span className="records-link-label">{row.website}</span>
                        <Icon size={12}>
          ...
          <tfoot>
            <tr className="records-calculation-row">
              ...
              <td className="records-cell">
                <button type="button" className="records-add-calculation">
                  <Icon size={15}>
                    <path d="M12 5v14M5 12h14" />
                  </Icon>
                  Add calculation
              ...
              <td className="records-cell">
                <span className="records-footer-value records-average">
                  <span className="records-strength-dot" style={{ background: "var(--orange)" }} />
                  {Math.round(
      ...
      {prop && meta && (
        ...
        >
          ...
          <ConfigRow label="Type">
            ...
            >
              <span className="text-ink-2">
                <Icon size={14}>{TYPE_GLYPHS[meta.type] ?? TYPE_GLYPHS.Text}</Icon>
              </span>
              {meta.type}
            ...
            {configMenu === "tool" && (
              <ConfigPicker
                ...
                onSelect={(tool) => {
                  setColumnOverrides((current) => ({
                    ...current,
                    [prop.col]: { ...current[prop.col], tool, toolKind: "model" },
                  }));
                  setConfigMenu(null);
                }}
              />
          ...
          <ConfigRow label="Grounding">
            ...
            {groundingHelpOpen && (
              <div
                className="absolute right-0 top-[30px] z-30 w-[230px] rounded-[10px] px-3 py-2.5 text-[12px] leading-relaxed shadow-overlay"
                style={{ color: "var(--tooltip-fg)", background: "var(--tooltip-bg)" }}
                role="status"
              >
              ...
              {selectedInputs.length ? (
                <span className="flex min-w-0 items-center gap-1">
                  {selectedInputs.slice(0, 2).map((input) => (
                    <span
                      key={input}
                      className="max-w-[92px] truncate rounded-[5px] bg-accent-tint px-1.5 py-0.5 text-[12px] font-medium text-accent-ink"
                    >
                      {input}
                    </span>
                  ))}
              ...
              <span className="text-ink">
                ...
                {meta.prompt.chip && (
                  <span
                    ...
                    className="rounded-[5px] bg-accent-tint px-1.5 py-0.5 text-[12px] font-medium text-accent-ink"
                  >
                    {meta.prompt.chip}
                  </span>
                )}
          ...
          >
            ...
            >
              <span className={pinnedColumns.has(prop.col) ? "text-accent" : "text-ink-2"}>
                <Icon size={15}>
            ...
            <button
              ...
              className="relative z-10 -mx-1.5 flex h-8 items-center gap-2.5 rounded-[8px] px-1.5 text-left text-[13px] leading-none text-ink transition-transform duration-150 active:scale-[0.96]"
            >
              <span className={moreSettingsOpen ? "text-ink" : "text-ink-2"}>
                <Icon size={15}>
                  <g>
            ...
            >
              ...
              <ConfigRow label="Required value">
                <MiniSwitch
                  ...
                  onToggle={() =>
                    setAdvancedSettings((current) => ({ ...current, required: !current.required }))
                  }
                />
              </ConfigRow>
