...
/* The vendored `glimm` shader package is not part of Grove's dependency
 * contract, so the flagship-model celebration is a one-shot rainbow band
 * swept across the composer with the 2D canvas API instead. Same trigger,
 * same direction and timing as the original sweep (570ms, easeOutExpo). */
const RAINBOW = ["#ff5a5a", "#ff9d43", "#ffd93d", "#4cd964", "#3dc9ff", "#5a8dff", "#a06bff"];
...
function playRainbowSweep(canvas: HTMLCanvasElement) {
  ...
  RAINBOW.forEach((color, i) => gradient.addColorStop(i / (RAINBOW.length - 1), color));
  const frame = (now: number) => {
    const t = Math.min(1, (now - start) / SWEEP_MS);
    const eased = easeOutExpo(t);
    ctx.clearRect(0, 0, width, height);
    const x = -bandWidth + eased * (width + bandWidth * 2);
    /* fade the band in and out over the sweep so the edges never hard-clip */
...
export default function PromptBar({
  ...
  /* The rainbow sweep lives inside the composer, invisible at rest.
   * Selecting the flagship model fires a one-shot sweep across the interior. */
  const celebrate = () => {
